...
public class GlobalExceptionHandler {
    ...
    private ErrorResponse handleException(Exception ex, WebRequest request) {
        ...
        if ("GET".equalsIgnoreCase(reqMethod)) {
            requestParams =
                    Collections.list(servletRequest.getParameterNames()).stream()
                            .map(pn -> pn + " : "
                                    + Arrays.toString(Optional
                                            .ofNullable(servletRequest.getParameterValues(pn))
                                            .orElse(new String[0])))
                            .collect(Collectors.joining(", "));
        } else if ("POST".equalsIgnoreCase(reqMethod)) {
            try {
                requestParams = servletRequest.getReader().lines()
        ...
        log.warn("Error: [{}], Url: [{}], Parameters: [{}]", ex.getMessage(),
                servletRequest.getRequestURI(), requestParams);
